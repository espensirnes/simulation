	...
	def receive(self):
		...
		try:
			...
			if e.args[0]=='Ran out of input':
				raise RuntimeError("""An error occured in one of the spawned sub-processes. Check the output in "slave_errors.txt' in your working directory or run without multiprocessing""")
			else:
...
class multiprocess:
	def __init__(self,max_nodes=None,modules=[],run_multiprocess=True):
