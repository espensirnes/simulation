...
def main():
	...
	for mixed_norm in [0,1,2]:
		for adj,d,name in  [[False,0.00316157992580004,'discrete'],[False,0,'unadjusted'],[False,0,'continous']]:
			for cluster in [1,960]:
				if not (mixed_norm!=1 and cluster>1):
					fname="%s%s_%s" %(name,mixed_norm,cluster)
					simulation(mproc,200,d,fname,adj,mixed_norm,cluster)
...
def simulation(mproc,nsims,d,name,adj,mixed_norm,cluster):
	...
	sds=    [0.00316157992580004/25
	         ,3.60098707366965E-05, 	4.94210799812644E-05, 	5.79468667045995E-05, 	6.53806107798397E-05, 	
	...
	if d==0:
		r=run_sims(mproc,sds[2],28800,d,nsims,windows,adj,mixed_norm,cluster,sds[16],p,sd_arr,sds)
		a.append(r)		
	else:
		for i in range(0,len(sds)):
			r=run_sims(mproc,sds[i],28800,d,nsims,windows,adj,mixed_norm,cluster,sd,p,sd_arr,sds)
			a.append(r)
...
def run_sims(mproc,sd,periods,d,nsims,windows,adj,mixed_norm,cluster,minsd,p,sd_arr,sds):
	a=[]
	r=[]
	cnt_d=dict()
	...
	if not mproc is None:
		...
		for i in windows:
			a.append('r%s=sim.run_sims_window(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)' %(i,i,sd,periods,d,nsims,adj,mixed_norm,cluster,minsd,'p','sd_arr','sds'))	
		...
		for i in windows:
			m=sim.run_sims_window(i,sd,periods,d,nsims,adj,mixed_norm,cluster,minsd,p,sd_arr,sds)
			r.append(m)
	...
	names=names+['msq_exp_ineff','abs_exp_ineff']
